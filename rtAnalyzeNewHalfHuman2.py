...
def plot_brs_sequences_raw(t_rr, rr_intervals, sbp_values, brs_results):
    """
    绘制原始的RR间期和SBP值，并在图中标记BRS序列的位置。
    ...
    color = 'tab:blue'
    ax1.set_xlabel('时间 (s)')
    ax1.set_ylabel('RR间期 (ms)', color=color)
    ...
    for result in brs_results:
        ...
        for sbp_seq, rr_seq in seqs:
            ...
            if indices:
                ...
                # 在SBP图上标记BRS序列
                ax2.plot(t_rr[indices], sbp_values[indices], 'ro', markersize=8)
    ...
    plt.title('原始RR间期和SBP值中的BRS序列标记')
    plt.tight_layout()
    plt.show()
    
def plot_brs_sequences_subplots(t_ecg, rr_signal, sbp_signal, brs_results):
    """
    将RR间期、SBP信号和BRS序列标记分别绘制在三个子图中。

    参数：
    t_ecg -- 原始ECG信号的时间数组
    rr_signal -- 方波形式的RR间期信号
    sbp_signal -- 方波形式的SBP信号
    brs_results -- compute_brs_sequences函数的输出结果
    """
    ...
    # 子图1：绘制RR间期信号
    ax1 = axes[0]
    ax1.plot(t_ecg, rr_signal, color='tab:blue', label='RR间期')
    ax1.set_ylabel('RR间期 (ms)')
    ax1.legend(loc='upper right')
    ax1.set_ylim(600,1500)
    ax1.set_title('RR间期信号')

    # 子图2：绘制SBP信号
    ax2 = axes[1]
    ax2.plot(t_ecg, sbp_signal, color='tab:red', label='SBP值')
    ax2.set_ylabel('SBP值')
    ...
    # 子图3：绘制RR间期和SBP信号，并标记BRS序列
    ax3 = axes[2]
    ax3.plot(t_ecg, rr_signal, color='tab:blue', label='RR间期')
    ax3.plot(t_ecg, sbp_signal, color='tab:red', label='SBP值')
    ax3.set_ylabel('信号值')
    ax3.set_xlabel('时间 (s)')
    # ax3.set_ylim(600,1500)
    
    ax3.legend(loc='upper right')
    ax3.set_title('RR间期和SBP信号中的BRS序列标记')
    ...
    for result in brs_results:
        ...
        for sbp_seq, rr_seq in seqs:
            indices = []
            for i in range(len(rr_seq)):
                idx = np.where((rr_signal == rr_seq[i]) & (sbp_signal == sbp_seq[i]))[0]
                if len(idx) > 0:
                    indices.append(idx[0])
            if indices:
                ax3.plot(t_ecg[indices], rr_signal[indices], 'ro', markersize=8, label='BRS序列' if 'BRS序列' not in ax3.get_legend_handles_labels()[1] else "")
...
def plot_brs_sequences(t_rr, rr_intervals, sbp_values, brs_results):
    ...
    plt.tight_layout()
    plt.show()
def compute_brs_sequences(rr_intervals, sbp_values, window_size=10, step_size=1):
    """
...
for i in range(len(rr_intervals)):
    ...
    if len(sbp_in_interval) == 0:
        ...
        sbp_mean = sbp_at_rr[-1] if i > 0 else np.mean(sbp_values)
    else:
...
# 绘制原始的RR间期和SBP值，并标记BRS序列
plot_brs_sequences_raw(t_rr, rr_intervals, sbp_at_rr, brs_results)