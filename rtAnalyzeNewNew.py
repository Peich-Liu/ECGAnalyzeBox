...
t_ecg = peaks_ecg / sampling_rate  # R峰的时间位置

# 计算RR间期（以毫秒为单位）
rr_intervals = np.diff(t_ecg) * 1000  # RR间期（毫秒）
t_rr = t_ecg[1:]  # RR间期对应的时间

# 检测ABP的SBP峰值位置
peaks_abp, _ = find_peaks(filtered_abp, distance=int(sampling_rate * 0.5))
t_abp = peaks_abp / sampling_rate  # SBP峰的时间位置
sbp_values = filtered_abp[peaks_abp]

# 为每个RR间期找到对应的SBP值（使用最大值）
sbp_at_rr = []
...
plot_brs_sequences(t_rr, rr_intervals, sbp_at_rr, brs_results)