...
def compute_brs_sequence_with_drift(rr_intervals, sbp_values, brs_sequences, loop_num):
    """
    使用序列法计算BRS。
...
def main():
    ...
    for i in range(0, len(ecg_signal), 100000):
        ...
        brs_sequences, mean_brs = compute_brs_sequence_with_drift(rr_intervals, sbp_values, brs_sequences, i)
        rr_intervals -= np.mean(rr_intervals)

        # 存储结果
        results["rr_intervals"].append(rr_intervals)
        results["rr_times"].append(rr_times)
        results["sbp_values"].append(sbp_values)
        results["sbp_times"].append(sbp_times)
        # results["r_peaks_times"].append(r_peaks_times)

        # results["brs_sequences"].append(brs_sequences)
        # results["mean_brs"].append(mean_brs)
        print(len(results["rr_intervals"]))
        # print("平均BRS:", mean_brs)
    print(brs_sequences)

    all_rr_intervals = np.concatenate(results["rr_intervals"])
    all_rr_times = np.concatenate(results["rr_times"])
    all_sbp_values = np.concatenate(results["sbp_values"])
    all_sbp_times = np.concatenate(results["sbp_times"])
    ...
    sbp_indices = np.clip(sbp_indices, 0, len(filtered_ap_signal) - 1)  # 防止索引越界
