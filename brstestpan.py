...
def pan_tompkins(ecg_signal, fs):
    """
    使用 Pan-Tompkins 算法处理 ECG 信号，检测 R 波峰值并计算 RR 间期。
    ...
    diff_ecg = np.convolve(filtered_ecg, derivative_filter, mode='same')

    # 3. 平方
    squared_ecg = diff_ecg ** 2

    # 4. 移动窗口积分
    window_size = int(0.15 * fs)  # 150ms 的窗口
    window = np.ones(window_size) / window_size
    integrated_ecg = np.convolve(squared_ecg, window, mode='same')

    # 5. 峰值检测
    threshold = np.mean(integrated_ecg) * 0.5
    peaks = []
    refractory_period = 0.2 * fs  # 不应期为 200ms
    last_peak = -np.inf
    for i in range(len(integrated_ecg)):
...
def main():
    ...
    # 从ECG信号中检测R峰
    r_peaks_indices, r_peaks_times = pan_tompkins(ecg_signal, fs)
    ...
    plt.plot(range(len(r_peaks_indices)), ecg_signal[r_peaks_indices], 'ro', label='R peak')
